...
export function useCreateInvite() {
  ...
  return useMutation({
    ...
    onMutate: async (coachId: string) => {
      ...
      qc.setQueryData<Invite[]>(queryKeys.invites.byCoach(coachId), (old) => [
        optimistic,
        ...(old ?? []),
      ]);
...
export function useRevokeInvite() {
  ...
  return useMutation({
    mutationFn: ({ inviteId }: { inviteId: string; coachId: string }) => revokeInvite(inviteId),
    onMutate: async ({ inviteId, coachId }) => {
      ...
      qc.setQueryData<Invite[]>(
        ...
        (old) => old?.map((i) => (i.id === inviteId ? { ...i, status: 'revoked' } : i)) ?? [],
      );
