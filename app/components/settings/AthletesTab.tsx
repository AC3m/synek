...
export function AthletesTab({ className }: AthletesTabProps) {
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <Button onClick={handleGenerate} disabled={limitReached || createInvite.isPending} size="sm">
        {createInvite.isPending ? t('athletes.generating') : t('athletes.generateLink')}
      </Button>

      {limitReached && <p className="text-sm text-destructive">{t('athletes.limitReached')}</p>}
                ...
                {invite.status === 'pending' && (
                  <>
                    ...
                    <Button
                      ...
                      onClick={() => revokeInvite.mutate({ inviteId: invite.id, coachId })}
                    >
