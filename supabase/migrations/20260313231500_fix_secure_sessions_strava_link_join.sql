-- Repair stale Strava linkage and make secure_training_sessions resilient.
-- Some historical rows can have training_sessions.strava_activity_id populated
-- while strava_activities.training_session_id is missing/outdated.

-- Self-heal ownership/linkage using the canonical strava_id reference stored on training_sessions.
...
WHERE ts.strava_activity_id IS NOT NULL
  AND sa.strava_id = ts.strava_activity_id
  AND (
    sa.training_session_id IS DISTINCT FROM ts.id
    OR sa.user_id IS DISTINCT FROM wp.athlete_id
  );
