-- ============================================================
-- Fix: Remove SECURITY DEFINER so auth.uid() resolves correctly
-- ============================================================

CREATE OR REPLACE FUNCTION confirm_all_strava_sessions_for_week(p_week_plan_id UUID)
RETURNS void
LANGUAGE plpgsql
-- SECURITY INVOKER runs with the privileges of the user calling the function,
-- which means auth.uid() will be correctly populated from their JWT.
SECURITY INVOKER
